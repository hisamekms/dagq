...
- **inbox・plannerの区間（hook）**（ADR-0048の決定6・7、task 387）: runtimeがheadlessで起動しないinbox・人が`dagq plan`で開くplanner・runtimeが立てるplannerの区間は、pluginの`SessionStart`（matcherなしで`startup` / `resume` / `clear` / `compact`の全部）と`SessionEnd`のhook（`hooks/session-event.sh open|close`、[plugin-integration](plugin-integration.md#sessionの区間hookadr-0048)）が隠しコマンド`dagq session-event open|close`で記録する。CLIはhookのstdin（`session_id`・`transcript_path`・`cwd`・`source` / `reason`）と環境変数（kindの`DAGQ_SESSION_KIND`、無い古いworkspaceは`DAGQ_ROLE`から`inbox` / `planner`、`DAGQ_PLANNER_ORIGIN=runtime`なら`runtime_planner`。workspaceの`CMUX_WORKSPACE_ID`、plannerの`DAGQ_PLANNER_ID`）を`domain::sessions::SessionHook`にし、`domain::sessions::hook_changes`（純粋関数）が開いている3つのkindの区間から開き閉じるものを決め、`infrastructure::sessions::record_hook`が1つの書き込みトランザクションで書く（閉じる区間のtranscriptはその前に読む）。`up`はinboxのworkspaceに`DAGQ_SESSION_KIND=inbox`、`dagq plan`は`planner`、runtimeが立てるplannerは`runtime_planner`を`--env`で置く。
  - 同じsession_idの区間が開いていれば（`resume`・`compact`・session_idの変わらない`clear`）何も書かず、その区間を続ける。別のsession_idの`SessionStart`なら、同じworkspaceで開いている区間を`next_span`で閉じてから新しい区間を開く（`/clear`の新しいsession_id。`SessionEnd`（`reason: clear`）が先に来ればそれが閉じる）。`SessionEnd`はそのsession_idの区間を閉じ、`reason`は`clear` / `logout`はそのまま、ほか（`prompt_input_exit` / `other`など）は`exited`。閉じた区間への2回目の終了は何も書かないので、`/clear`の`SessionEnd`と次の`SessionStart`の両方が来ても区間は1回だけ閉じ、二重に数えない。
  - `session_opened`のpayloadは`kind`・`session_id`・`cwd`・`transcript_path`（hookが渡したもの。稼働時間はこのtranscriptから読む）・`workspace_id`・`provider`（`claude`）・`source`、plannerは`planner_id`、`runtime_planner`はそのplannerの`proposal_id`と`goal_ids`（`stats`の`--goal`に使う）。proposalの無いruntimeのplanner（draftのplanner）とinbox・人のplannerはtaskの無いevent。
  - hookのCLIは区間のeventだけを書き、run・proposal・plannerの状態を変えない。開いている区間の完了したturnはsupervisorの取り込み（下の[transcriptと稼働時間](#transcriptと稼働時間)）が記録し、閉じるときに残りを記録して`active_secs`を持つ。
  - **推定で閉じる**: supervisorは取り込みのたび（既定10分ごとと、observerの起動前）に、hookが記録した開いている区間のworkspaceが`cmux workspace list`（`listed_workspace_ids`）に居るかを見て、居なければ`inferred`で閉じる（`RunStore::close_gone_sessions`）。区間は一覧を取る前に読むので、その後に開いた区間を消えたものと取り違えない。時刻はtranscriptの最後のレコード（読めなければ閉じた時刻）。workspaceを持たない区間と、一覧を取れなかったときは閉じない。失敗はlogだけ。
...
- **対象**: `worker` / `resume` / `revise`と、headlessのjobの`review` / `triage` / `plan_review` / `observer`、hookが記録する`inbox` / `planner` / `runtime_planner`（task 387）。復旧（`recover`）のjobは区間を持たない
